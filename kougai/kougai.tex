...
膝関節への負担軽減を目的とする場合には，体幹をやや前傾させ，脛骨をできるだけ鉛直に保つ股関節主導のフォームとすることで，膝関節モーメントを低減できることが報告されている\cite{squat}．
...
MediaPipeを用いることで，カメラ映像から人体の関節位置をリアルタイムに取得でき，各関節の角度や重心の位置といった動作パラメータ，各関節間の比率を取得することが可能である．
...
MediaPipeから取得した各関節の角度や重心の位置といった動作パラメータ，各関節間の比率からフォームと重心の位置が利用者の目的に適しているか判定するシステムを構築する．
判定にあたっては，スクワット動作中の下肢の運動学的特徴に関する先行研究\cite{squat}を参考に，各目的に適したフォームと重心の位置を定める．

利用者に提示するフィードバックの設計においては，マルチモーダルフィードバックに関する知見\cite{foam}を参考に，利用者が無理なく自然にフォーム修正を行えるよう，フィードバックの提示方法について検討を行う．

これらの要素を統合し，利用者が膝への負担軽減を目的とする場合と腰への負担軽減を目的とする場合とで，それぞれ異なる修正方向を提示できるフィードバックシステムの構築を行う．
